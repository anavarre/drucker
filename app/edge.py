...
def create_base2edge_container(drucker):
    ...
    print(
        colorful.white_on_blue(
            "Spinning up %s container with ID:" % (drucker.vars.EDGE_CONTAINER)
        )
    )
...
def create_edge_image(drucker):
    ...
    print(
        colorful.white_on_blue(
            ...
            % (drucker.vars.EDGE_IMAGE, drucker.vars.EDGE_CONTAINER)
        )
    )
    ...
    subprocess.getoutput(
        "docker rm -f %s > /dev/null 2>&1" % (drucker.vars.EDGE_CONTAINER)
    )
...
def start_edge_container(drucker):
    ...
    subprocess.getoutput(
        "docker start %s > /dev/null 2>&1" % (drucker.vars.EDGE_CONTAINER)
    )
...
def provision_edge_container(drucker):
    ...
    if subprocess.getoutput(
        "docker ps -a | grep -o %s" % (drucker.vars.EDGE_CONTAINER)
    ):
        print(
            colorful.green(
                "%s container already exists." % (drucker.vars.EDGE_CONTAINER)
            )
        )

        if subprocess.getoutput(
            "docker ps | grep -o %s" % (drucker.vars.EDGE_CONTAINER)
        ):
            o.run_edge_orchestration(drucker)
        else:
            print(
                colorful.white_on_blue(
                    "Starting %s container..." % (drucker.vars.EDGE_CONTAINER)
                )
            )
        ...
        if subprocess.getoutput(
            ...
            print(
                colorful.green(
                    "%s custom image already exists." % (drucker.vars.EDGE_IMAGE)
                )
            )
            create_edge_container(drucker)
        else:
