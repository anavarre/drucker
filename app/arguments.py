...
def return_version(drucker):
    ...
    latest_commit = subprocess.getoutput(
        "cd %s && git rev-parse --short HEAD" % (drucker.vars.APP_ROOT)
    )
...
def get_parser():
    ...
    parser.add_argument("app", nargs="?", help="Targets an arbitrary app")
    parser.add_argument(
        "--drupal",
        dest="dispatched_function",
        action="store_const",
