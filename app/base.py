...
def create_base_image():
    ...
    s.run("docker commit -m \"%s on %s\" %s %s" % (drucker.vars.BASE_CONTAINER,
                                                   str(date.today()),
                                                   drucker.vars.BASE_CONTAINER,
...
def delete_base_container():
    ...
    s.getoutput("docker rm -f %s > /dev/null 2>&1" % (drucker.vars.BASE_CONTAINER))


def delete_init_image():
    """Delete init image"""
    print(c.blue("Deleting %s image..." % (drucker.vars.INIT_IMAGE)))
    s.getoutput("docker rmi %s > /dev/null 2>&1" % (drucker.vars.INIT_IMAGE))
...
def provision_base_container(drucker):
    ...
    if s.getoutput(drucker.vars.CHECK_BASE_IMAGE):
        print(c.green("%s image already exists." % (drucker.vars.BASE_IMAGE)))

        if s.getoutput(drucker.vars.CHECK_INIT_IMAGE):
            delete_init_image()
    else:
        if s.getoutput("docker ps -a | grep -o \"%s\"" % (drucker.vars.BASE_CONTAINER)):
