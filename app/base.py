...
def create_base_container(drucker):
    ...
    print(
        colorful.white_on_blue(
            "Spinning up %s container with ID:" % (drucker.vars.BASE_CONTAINER)
        )
    )
    subprocess.run(
...
def create_base_image(drucker):
    ...
    print(
        colorful.white_on_blue(
            ...
            % (drucker.vars.BASE_IMAGE, drucker.vars.BASE_CONTAINER)
        )
    )
    subprocess.run(
...
def delete_base_container(drucker):
    ...
    print(
        colorful.white_on_blue(
            "Deleting %s container..." % (drucker.vars.BASE_CONTAINER)
        )
    )
    subprocess.getoutput(
        "docker rm -f %s > /dev/null 2>&1" % (drucker.vars.BASE_CONTAINER)
    )
...
def provision_base_container(drucker):
    ...
    if subprocess.getoutput(drucker.vars.CHECK_BASE_IMAGE):
        ...
        if subprocess.getoutput(
            'docker ps -a | grep -o "%s"' % (drucker.vars.BASE_CONTAINER)
        ):
