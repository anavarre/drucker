...
def create_base2mirror_container(drucker):
    ...
    print(
        colorful.white_on_blue(
            "Spinning up %s container with ID:" % (drucker.vars.MIRROR_CONTAINER)
        )
    )
...
def create_mirror_image(drucker):
    ...
    print(
        colorful.white_on_blue(
            ...
            % (drucker.vars.MIRROR_IMAGE, drucker.vars.MIRROR_CONTAINER)
        )
    )
    ...
    subprocess.getoutput(
        "docker rm -f %s > /dev/null 2>&1" % (drucker.vars.MIRROR_CONTAINER)
    )
...
def start_mirror_container(drucker):
    ...
    subprocess.getoutput(
        "docker start %s > /dev/null 2>&1" % (drucker.vars.MIRROR_CONTAINER)
    )
...
def provision_mirror_container(drucker):
    ...
    if subprocess.getoutput(
        "docker ps -a | grep -o %s" % (drucker.vars.MIRROR_CONTAINER)
    ):
        print(
            colorful.green(
                "%s container already exists." % (drucker.vars.MIRROR_CONTAINER)
            )
        )
        if subprocess.getoutput(
            "docker ps | grep -o %s" % (drucker.vars.MIRROR_CONTAINER)
        ):
            o.run_mirror_orchestration(drucker)
        else:
            print(
                colorful.white_on_blue(
                    "Starting %s container..." % (drucker.vars.MIRROR_CONTAINER)
                )
            )
        ...
        if subprocess.getoutput(
            ...
            print(
                colorful.green(
                    "%s custom image already exists." % (drucker.vars.MIRROR_IMAGE)
                )
            )
            create_mirror_container(drucker)
        else:
