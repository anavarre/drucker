...
def create_base2search_container(drucker):
    ...
    print(colorful.white_on_blue("Spinning up %s container with ID:" % (drucker.vars.SEARCH_CONTAINER)))
...
def provision_search_container(drucker):
    ...
    if subprocess.getoutput("docker ps -a | grep -o %s" % (drucker.vars.SEARCH_CONTAINER)):
        ...
        if subprocess.getoutput("docker ps | grep -o %s" % (drucker.vars.SEARCH_CONTAINER)):
            ...
            print(colorful.white_on_blue("Starting %s container..." % (drucker.vars.SEARCH_CONTAINER)))
