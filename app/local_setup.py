...
def set_local_ssh_path(drucker):
    ...
    if drucker.vars.KEY_PLACEHOLDER in open(drucker.vars.DEFAULT_CONFIG).read():
        pubkey = input("Enter path to SSH public key (%s): " % (drucker.vars.DEFAULT_PUBKEY))

        if not pubkey:
            replace_string(drucker.vars.DEFAULT_CONFIG, drucker.vars.KEY_PLACEHOLDER, drucker.vars.DEFAULT_PUBKEY)
...
def set_local_html_path(drucker):
    ...
    if drucker.vars.HTML_PLACEHOLDER in open(drucker.vars.DEFAULT_CONFIG).read():
        ...
        Where should we store sites locally? (%s): ''' % (drucker.vars.DEFAULT_HTML_PATH))

        if not host_html_path:
            replace_string(drucker.vars.DEFAULT_CONFIG, drucker.vars.HTML_PLACEHOLDER, drucker.vars.DEFAULT_HTML_PATH)
...
def set_local_db_path(drucker):
    ...
    if drucker.vars.DB_PLACEHOLDER in open(drucker.vars.DEFAULT_CONFIG).read():
        host_db_path = input('''
        Where should we store databases locally? (%s): ''' % (drucker.vars.DEFAULT_DB_PATH))

        if not host_db_path:
            replace_string(drucker.vars.DEFAULT_CONFIG, drucker.vars.DB_PLACEHOLDER, drucker.vars.DEFAULT_DB_PATH)
